...
public class SupplierService {
  ...
  public Page<SupplierDTO> getAllSuppliers(int page, int size, @NotNull String name) {
    ...
    Specification<Supplier> specification = SupplierSpecification.searchByName(name)
        .and(SupplierSpecification.isDeleted());
    return supplierRepository.findAll(specification, pageable).map(
        s -> new SupplierDTO(s.getSupplierId(), s.getName(), s.getAddress(), s.getPhone(), s.getEmail()));
  }
  ...
  public SupplierDTO addSupplier(SupplierDTO dto) {
    ...
    return new SupplierDTO(saved.getSupplierId(), saved.getName(), saved.getAddress(), saved.getPhone(),
        saved.getEmail());
  }
  ...
  public SupplierDTO updateSupplier(Long id, SupplierDTO dto) {
    ...
    if (supplierOptional.isPresent()) {
      ...
      return new SupplierDTO(supplierUpdated.getSupplierId(), supplierUpdated.getName(), supplierUpdated.getAddress(),
  ...
  public void deletedSupplier(Long id) {
    Optional<Supplier> supplierOptional = supplierRepository.findById(id);
    supplierOptional.ifPresent(sup -> {
