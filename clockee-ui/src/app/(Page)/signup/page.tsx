...
import Header_signup from "@/app/components/header/header_signup";
...
export default function RegisterUser() {
  ...
  return (
    <div className="w-full min-h-screen flex flex-col items-center bg-white">
      {/* Header */}
        <div className="bg-[#FFDA3E]  w-full max-w-6xl flex justify-between items-center ">
            <Header_signup/>
        </div>
      {/* Main Content */}
            ...
            {/* Form Register */}
              <div className="w-full md:w-[380px] bg-white p-8 rounded-lg shadow-lg border border-yellow-300">
                ...
                {/* Form Đăng ký */}
                  ...
                  />
                  {email && !isValidEmail(email) && <p className="text-red-500 mt-2 text-sm">Email không hợp lệ!</p>}
                  <input
                    type="text"
                  ...
                  />
                  {fullName.trim() === "" && <p className="text-red-500 mt-2 text-sm">Vui lòng nhập họ và tên!</p>}  
                  <input
                    type="text"
                  ...
                  />
                  {phoneNumber && !isValidPhone(phoneNumber) && <p className="text-red-500 mt-2 text-sm">Số điện thoại chưa hợp lệ!</p>} 
                {/*Xử lý mật khẩu nhập*/}
                  <div className="relative">
                      ...
                      {isPasswordValid && <CheckCircle size={20} className="absolute inset-y-0 top-[22px] right-10 text-green-500" />}  
                  </div> 
                {/* Cảnh báo nếu nhập chưa đủ 6 ký tự */}
                  {password && !isPasswordValid && (<p className="text-red-500 mt-2 text-sm">Mật khẩu phải có ít nhất 6 kí tự!</p>)}   
                {/* Cảnh báo nếu mật khẩu chứa dấu tiếng việt */}
                  {password && !isPasswordNoDiacritics && (<p className="text-red-500 mt-2 text-sm">Mật khẩu không được có dấu!</p>)}
                {/* Xử lý nhập lại mật khẩu */}
                  <div className="relative">
                      ...
                      {passwordMatch && isPasswordValid && isPasswordNoDiacritics && (
                        <CheckCircle size={20} className="absolute inset-y-0 top-[22px] right-10 text-green-500" />)}
                  </div>
                {/* Cảnh báo nếu mật khẩu nhập lại không khớp*/}
                  {confirmPassword && !passwordMatch && isPasswordValid && isPasswordNoDiacritics && (<p className="text-red-500 mt-2 text-sm">Mat khau khong khop!</p>)}
                ...
                {/* Link quên mật khẩu & đăng ký */}
                  <div className="flex justify-center text-sm text-gray-600 mt-3 border-t border-gray-300 pt-4">
                      ...
                      <Link href={"/login"} className="text-yellow-700 cursor-pointer hover:text-yellow-900">Đăng nhập</Link>
                  </div>
              </div>
          </main>
        </div>
