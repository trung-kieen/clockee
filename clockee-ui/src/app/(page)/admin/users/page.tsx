...
import { AdminUserControllerService, PageResponseUserDetailResponse, UserDetailResponse } from "@/gen";
...
export default function UserAdminPage() {
  ...
  try {
    ...
    const roleResults = await Promise.all(
      users.map(async (user) => {
        try {
          const roles = await AdminUserControllerService.getRoleById(user.userId!);
          return {
            userId: user.userId,
            role: roles.map((r) => r.roleName).join(", ") || "Không có vai trò",
          };
        } catch {
  ...
  const {pageInfo, setPage, query, setQuery, setPageInfo, page } = usePageSearch<PageResponseUserDetailResponse>({
