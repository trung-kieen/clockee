...
import { AdminOrderSummaryResponse, AdminProductControllerService, OrderSummaryResponse, ProductDetailsResponse } from "@/gen";
...
const OrderDetail = () => {
  ...
  const fetchProduct = async (Ids: number[]) => {
    const productPromises = Ids.map((id) => AdminProductControllerService.getProductById(id));
  ...
  useEffect( () => {
    const foundOrder = mockDetailOrderItems.find(o => o.order_id === order?.orderId);
    if(foundOrder) {
  ...
  <AdminMainCard goBack={true} title="Chi tiết đơn hàng">
    {order ? (
      <div className="bg-gray grid grid-row-3 gap-4">
        ...
        <div>
            <label className="block text-xl font-medium">Thời gian đặt hàng</label>
            <input
        ...
        <div className="w-ful grid grid-cols-2 gap-6">
          <div>
            <label className="block text-xl font-medium">Trạng thái đơn hàng</label>
            <input
              value={order.status || ""}
              disabled
              className="w-full border p-2 rounded mt-2 "
          ...
          <div className="mb-4">
            <label className="block text-xl font-medium">Tổng giá trị đơn hàng</label>
            <input
          ...
          <table className="table table-zebra w-full text-left ">
            ...
            <tbody>
              {orderItem.map((item,index)=> {
                const product: ProductDetailsResponse | undefined  = products.find((p) => p?.productId === item.product_id);
