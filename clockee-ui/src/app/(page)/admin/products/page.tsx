...
import { AdminProductControllerService, AdminProductResponse, PageAdminProductResponse } from "@/gen";
...
export default function ProductAdminPage() {
  ...
  const fetchProducts = async (page: number, query: string) => {
    try {
      const pageInfo = await AdminProductControllerService.getAllProducts(page - 1, undefined, query);
      if (pageInfo)
  ...
  const dataEntries = () => {
    ...
    if (!(pageInfo?.content) || pageInfo.empty) {
      ...
      return (
        <div>Hiện tại chưa có sản phẩm nào để hiển thị.</div>
      )
    }
    return (
      (
              ...
              <tbody>
                {
                  pageInfo.content.map((entry, idx) => {
                    return <ProductTableRow
                      key={idx}
                      item={entry} refreshCallBack={refresh} />
                  })
                }
              </tbody>
            </table>
    ...
    <AdminMainCard title="Sản phẩm" goBack={false}>
      ...
      <div className='flex flex-col gap-2 items-center'>
        ...
        <div className="relative flex items-center w-1/3">
          ...
          <input value={query} onChange={onChangeSearchQuery}
...
const ProductTableRow = ({ item, refreshCallBack }: ProductRowProps) => {
  ...
  return (
    <tr >
      ...
      <td onClick={() => { setIsOpenConfirm(true) }} className="hover:bg-gray-200">
        {/* Action delete */}
